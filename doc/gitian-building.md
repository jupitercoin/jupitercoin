...
*Setup instructions for a gitian build of Jupitercoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Jupitercoin
...
- [Building Jupitercoin](#building-jupitercoin)
...
Clone the git repositories for jupitercoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/jupitercoin/jupitercoin
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the jupitercoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building Jupitercoin
----------------

To build Jupitercoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the jupitercoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/jupitercoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/jupitercoin/jupitercoin
...
For example:
```bash
URL=https://github.com/laanwj/jupitercoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit jupitercoin=${COMMIT} --url jupitercoin=${URL} ../jupitercoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit jupitercoin=${COMMIT} --url jupitercoin=${URL} ../jupitercoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit jupitercoin=${COMMIT} --url jupitercoin=${URL} ../jupitercoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/jupitercoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/jupitercoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/jupitercoin-osx-build.assert
```
...
[jupitercoin/gitian.sigs](https://github.com/jupitercoin/gitian.sigs/) repository, or if that's not possible create a pull request. 
