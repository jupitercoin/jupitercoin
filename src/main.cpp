...
#if defined(NDEBUG)
# error "Jupitercoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Jupitercoin Signed Message:\n";
...
CAmount GetMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetJupitercoinDustFee(tx.vout, ::minRelayTxFee);
...
static bool ReadBlockOrHeader(T& block, const CDiskBlockPos& pos)
{
    ...
    // Check the header
    // Jupitercoin: We don't necessarily have block height, so we depend on using the base parameters
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Jupitercoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("jupitercoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    //if (fEnforceBIP30) {
    // Jupitercoin: BIP30 has been active since inception
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
    ...
    // Jupitercoin: BIP16 has been active since inception
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            // Jupitercoin: BIP16 has been enabled since inception
            nSigOps += GetP2SHSigOpCount(tx, view);
            if (nSigOps > MAX_BLOCK_SIGOPS)
                return state.DoS(100, error("ConnectBlock(): too many sigops"),
                                 REJECT_INVALID, "bad-blk-sigops");
    ...
    CAmount blockReward = nFees + GetJupitercoinBlockSubsidy(pindex->nHeight, consensus, hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Jupitercoin: Version 2 enforcement was never used, reject from v3 softfork
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Jupitercoin: Block v2 was never enforced, so we trigger this against v3
