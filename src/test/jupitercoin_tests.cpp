...
#include "jupitercoin.h"
#include "main.h"
#include "test/test_bitcoin.h"

#include <boost/test/unit_test.hpp>

BOOST_FIXTURE_TEST_SUITE(jupitercoin_tests, TestingSetup)
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    for (nHeight = 0; nHeight <= 100000; nHeight++) {
        ...
        CAmount nSubsidy = GetJupitercoinBlockSubsidy(nHeight, params, prevHash);
        BOOST_CHECK(MoneyRange(nSubsidy));
        BOOST_CHECK(nSubsidy <= 10 * COIN);
        nSum += nSubsidy * nStepSize;
    }
    for (; nHeight <= 200000; nHeight++) {
        const Consensus::Params& params = mainParams.GetConsensus(nHeight);
        CAmount nSubsidy = GetJupitercoinBlockSubsidy(nHeight, params, prevHash);
        BOOST_CHECK(MoneyRange(nSubsidy));
    ...
    for (; nHeight < 400000; nHeight++) {
        const Consensus::Params& params = mainParams.GetConsensus(nHeight);
        CAmount nSubsidy = GetJupitercoinBlockSubsidy(nHeight, params, prevHash);
        CAmount nExpectedSubsidy = (10 >> (nHeight / 100000)) * COIN;
        BOOST_CHECK(MoneyRange(nSubsidy));
    ...
    CAmount nConstantSubsidy = GetJupitercoinBlockSubsidy(600000, params, prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);

    nConstantSubsidy = GetJupitercoinBlockSubsidy(700000, params, prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_difficulty_limit)
{
    ...
    BOOST_CHECK_EQUAL(CalculateJupitercoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1e00ffff);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_pre_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateJupitercoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1c15ea59);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateJupitercoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b671062);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_upper)
{
    ...
    BOOST_CHECK_EQUAL(CalculateJupitercoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b4e56b3);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_lower)
{
    ...
    BOOST_CHECK_EQUAL(CalculateJupitercoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b335358);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_rounding)
{
    ...
    BOOST_CHECK_EQUAL(CalculateJupitercoinNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b6558a4);
}
